...
class ConfigManager:
    ...
    def _load(self) -> WorkerNetConfig:
        ...
        if self.config_file.exists():
            try:
                ...
                # На этапе загрузки логгер еще может быть не инициализирован
    ...
    def console_output(self, value: bool):
        ...
        # Полностью перенастраиваем логгер с новыми параметрами
        logger.configure(**self.get_log_config())
    ...
    def cache_max_size(self) -> int:
        return self._config.cache_max_size
    
    @cache_max_size.setter
    def cache_max_size(self, value: int):
        self._config.cache_max_size = value
        cache = self._get_cache()
    ...
    def save(self) -> bool:
        ...
        Вызывайте этот метод после всех изменений, чтобы сохранить их постоянно.
        """
        try:
            self._save()
    ...
    def reset(self, save: bool = False) -> 'ConfigManager':
        ...
        Изменения применяются immediately к текущей сессии.
        
        Args:
            save: Если True, сохраняет изменения в файл
        """
        old_config = self._config
        self._config = WorkerNetConfig()
        
        # Принудительно применяем все настройки к компонентам
    ...
    def _apply_changes(self, old: WorkerNetConfig, new: WorkerNetConfig):
        ...
        # Применяем настройки кэша, если они изменились
        if (old.cache_enabled != new.cache_enabled or
            old.cache_max_size != new.cache_max_size):
            
            cache = self._get_cache()
            if old.cache_enabled != new.cache_enabled:
                if new.cache_enabled:
                    cache.enable()
                else:
                    cache.disable()
            
            if old.cache_max_size != new.cache_max_size:
                cache.set_max_size(new.cache_max_size)
    ...
    def _apply_all_changes(self):
        ...
        if cache_config['enabled'] != cache.is_enabled():
        ...
        if hasattr(cache, 'set_max_size'):
            cache.set_max_size(cache_config['max_size'])
    ...
    def get_cache_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'evict_threshold': self._config.cache_evict_threshold,
            'evict_percent': self._config.cache_evict_percent,
            'auto_save': self._config.cache_auto_save,
            'cache_dir': str(self.cache_dir),
            'evict_strategy': self._config.cache_evict_strategy
        }
    ...
    def get_client_config(self) -> Dict[str, Any]:
        """Возвращает настройки для клиента"""
        return {
    ...
    def show_config(self, return_string: bool = False) -> Optional[str]:
        ...
        Args:
            return_string: Если True, возвращает строку вместо вывода в лог
        """
        ...
        lines = [
            ...
            "-" * 60,
            "КЭШ:",
            f"  Включён: {config_dict['cache_enabled']}",
            f"  Макс. размер: {config_dict['cache_max_size']}",
            f"  Стратегия: {config_dict['cache_evict_strategy']}",
            f"  Порог очистки: {config_dict['cache_evict_threshold']}",
            f"  Процент удаления: {config_dict['cache_evict_percent']}",
            f"  Автосохранение: {config_dict['cache_auto_save']}",
            "-" * 60,
            "API КЛИЕНТ:",
    ...
    def update(self, save: bool = False, **kwargs) -> 'ConfigManager':
        ...
        Args:
            save: Если True, сохраняет изменения в файл
            **kwargs: Имена настроек и их новые значения
        """
